...
const luckyNewApi = require("./utils/luckyNewApi");
...
async function handleAccountTask(task) {
    ...
    const isLuckyNew = mailProvider === 'luckynew';
    ...
    if (mode === 'auto_signup' || mode === 'auto_autopay') {
        ...
        for (let attempt = 1; attempt <= maxLuckRetries; attempt++) {
            ...
            try {
                const providerName = isTMail ? 'T-Mail' : isLuckyNew ? 'Lucky New' : 'LuckMail';
                ...
                if (isTMail) {
                    ...
                    const lnDomains = userData.luckMailDomains ? userData.luckMailDomains.split(',').map(d => d.trim()).filter(Boolean) : [];
                    purchase = await luckyNewApi.purchaseEmail(userData.luckyNewApiKey, lnDomains);
                } else {
    ...
    if (mode.startsWith('auto_')) {
        otpFnProxy = async () => {
            const providerName = isTMail ? 'T-Mail' : isLuckyNew ? 'Lucky New' : 'LuckMail';
            logger.info(`[#${threadId}] Waiting for OTP from ${providerName} for ${currentEmail}...`);
            let code;
            if (isTMail) {
                code = await tMailApi.fetchVerificationCode(token, currentEmail, tmailBaseUrl);
            } else if (isLuckyNew) {
                code = await luckyNewApi.fetchVerificationCode(token, currentEmail, userData.luckyNewApiKey);
            } else {
                code = await luckMailApi.fetchVerificationCode(token, currentEmail, userData.luckMailApiKey);
            }
            if (!code) throw new Error(`Timeout fetching OTP from ${providerName}`);
            return code;
        };
    } else {
        otpFnProxy = async () => {
            ...
            if (mode === 'retry_autopay') {
                ...
                */
                ...
                if (!sRes.success) {
                    ...
                    if (purchaseId) { if (isLuckyNew) { luckyNewApi.cancelEmail(purchaseId) } else { luckMailApi.cancelEmail(purchaseId) } }
                    if (activeSlot) await releaseGopaySlot(otpServerUrl, activeSlot.id);
