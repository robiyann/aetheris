...
function initTelegram() {
    ...
    try {
        ...
        bot.on('message', async (msg) => {
            ...
            if (text === '🚀 Full Auto Plus') {
                ...
                const buttons = [
                    ...
                    [{ text: "🔥 Lucky New", callback_data: 'fullpro_luckynew' }]
                ];
                bot.sendMessage(chatId,
        ...
        bot.on('callback_query', async (query) => {
            ...
            if (data === 'fullpro_luckmail' || data === 'fullpro_tmail' || data === 'fullpro_luckynew') {

                const mailProvider = data === 'fullpro_luckmail' ? 'luckmail' : data === 'fullpro_luckynew' ? 'luckynew' : 'tmail';
                const providerName = mailProvider === 'luckmail' ? '🍀 LuckMail' : mailProvider === 'luckynew' ? '🔥 Lucky New' : '📬 T-Mail';
                bot.answerCallbackQuery(query.id).catch(() => { });
                bot.deleteMessage(chatId, query.message.message_id).catch(() => { });
                ...
                if (mailProvider === 'luckynew' && !uData.luckyNewApiKey) {
                    bot.sendMessage(chatId, "❌ <b>Lucky New API Key not set.</b>\nGo to ⚙️ My Settings to add your key first.", { parse_mode: "HTML", ...mainMenuKeyboard });
                    return;
                }
            ...
            if (data === 'edit_luckynew_key') {
                bot.answerCallbackQuery(query.id).catch(() => { });
                bot.deleteMessage(chatId, query.message.message_id).catch(() => { });
                const key = await askTelegramUser(chatId, `🔥 Enter your <b>Lucky New API Key</b>:\n<i>Send "-" to remove</i>`);
                if (!key) return;
                if (key.trim() === '-') {
                    db.saveUser(chatId, { luckyNewApiKey: null });
                    bot.sendMessage(chatId, `✅ <b>Lucky New API Key removed</b>.`, { parse_mode: 'HTML', ...mainMenuKeyboard });
                } else {
                    db.saveUser(chatId, { luckyNewApiKey: key.trim() });
                    bot.sendMessage(chatId, `✅ <b>Lucky New API Key saved</b>.`, { parse_mode: 'HTML', ...mainMenuKeyboard });
                }
                return;
            }
...
function sendSettingsMenu(chatId, userData) {
    ...
    const luckyNewKey = userData.luckyNewApiKey ? '✅ Set' : '⚠️ Not set';
    ...
    const text = `⚙️ <b>MY SETTINGS</b>\n━━━━━━━━━━━━━━━━━━\n` +
        ...
        `🔥 <b>Lucky New Key    :</b> <code>${luckyNewKey}</code>\n\n` +
    ...
    bot.sendMessage(chatId, text, {
        ...
        reply_markup: {
            inline_keyboard: [
                ...
                [{ text: "🔥 Lucky New API Key", callback_data: "edit_luckynew_key" }],
